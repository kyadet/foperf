...
index=0
/root/foperf/bin/client -startid ${index} -addr ${target_addr} -wake ${wake} -recvint ${j} -wakeint 200 -sport :7000 -dport :7001 -errorThreshold 6 -checkTimer 5000 1>> result_${cliaddr1} 2>&1 &
index=$(( $index + $wake ))
ssh $cliaddr2 "/root/foperf/bin/client -startid ${index} -addr ${target_addr} -wake ${wake} -recvint ${j} -wakeint 200 -sport :7000 -dport :7001 -errorThreshold 6 -checkTimer 5000 " 1>> result_${cliaddr2} 2>&1 &
index=$(( $index + $wake ))
ssh $cliaddr3 "/root/foperf/bin/client -startid ${index} -addr ${target_addr} -wake ${wake} -recvint ${j} -wakeint 200 -sport :7000 -dport :7001 -errorThreshold 6 -checkTimer 5000 " 1>> result_${cliaddr3} 2>&1 &
